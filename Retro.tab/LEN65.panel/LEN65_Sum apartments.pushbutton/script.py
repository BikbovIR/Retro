...
2. Заполнить параметры:
    2.1 ADSK_Тип помещения
    2.2 ADSK_Номер квартиры
    
После работы функции буду пересчитаны параметры:
...
"3" - лоджия  коэффициент - 1
"4" - балкон  коэффициент - 1
...
for apartment_num, rooms in dict_rooms.items():
    ...
    for room in rooms:
        ...
        room_RetroArea  = room.LookupParameter(p_name_RetroArea).AsDouble()
        room_area_m2    = UnitUtils.ConvertFromInternalUnits(room_RetroArea, UnitTypeId.SquareMeters)
